...
{{
    ...
    "type": "concept",                       // concept（概念）/ theorem（定理）/ proof（证明）/ example（例题/示例）/ note（说明/推导）
...
{markdown_content}"""
    ...
    # 立即保存原始响应到 debug 文件，确保解析失败也能拿到内容
    _debug_path = "debug_response.txt"
    with open(_debug_path, "w", encoding="utf-8") as _f:
        _f.write(result)
    print(f"  💾 原始响应已保存到 {_debug_path}（无论解析成功与否）")
...
def main():
    ...
    _input_path = str(_BASE_DIR / "高等数学改" / "第一章 函数、极限、连续" / "第一节 映射与函数" / "一、集合和映射.md")
    _toc_file   = str(_BASE_DIR / "目录.md")

    input_path  = args.input  if args.input  else _input_path
    output_path = args.output if args.output else None  # 稍后根据 section 自动命名
    ...
    # 自动生成输出文件名（section + segment）
    if output_path is None:
        safe_section = sanitize_filename(section)
        output_path = f"{safe_section}-segment.json"
    ...
    raw_response = None  # 提前初始化，确保 except 块中可访问
    ...
    try:
        ...
        # 保存解析后的干净数据（经 try_parse_json 修复，合法 JSON）
        save_file(output_path, json.dumps(parsed, ensure_ascii=False))
        ...
        print("\n原始响应已保存到 debug_response.txt，请查看该文件定位问题。")
